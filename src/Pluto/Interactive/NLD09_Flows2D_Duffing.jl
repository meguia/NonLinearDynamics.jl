...
begin
    ...
    Pkg.instantiate()
    using NonLinearDynamics: flow2d_vectorfield, flow2d_nullclines, flow2d_manifolds, attractor_basin
...
which has a trivial solution $x^*_1=0$ 
 and, for $\beta>0$, two additional real solutions (obtained by dividing by the nonzero $x$):
$x^*_{2,3}=\pm\sqrt{\beta}$. At $\beta=0$ all three coalesce at the origin.
"""
...
for $\gamma\ne0$ determines a cubic curve in the plane:

$y = \beta x/\gamma - x^3/\gamma$

For $\gamma=0$, the second nullcline instead consists of the vertical lines where $\beta x-x^3=0$.
...
To visualize this without having to calculate it by hand we will use the function flow2d_nullclines which in addition to plotting the nullclines curves $\dot{x}=0$
 in red and $\dot{y}=0$
...
# ╔═╡ 76a4b3c9-7c37-5701-b661-dd9cdfe7690e
md"""
...
# ╔═╡ fab1f067-ac1b-5f13-80e1-17ee5d691fde
let parameters=[γ,β], xs=range(-1.6,1.6;length=401)
    gamma,beta=parameters
    a=plot(xs,xs.^4 ./4 .-beta.*xs.^2 ./2;xlabel="x",ylabel="V(x)",legend=false)
    b=flow2d_nullclines(duffing!,parameters;xlims=[-1.6,1.6],ylims=[-1.5,1.5],
...
# ╔═╡ 5ed75d17-0efe-555e-9cfe-7da2c4fba42a
let
    gamma,beta=[γ,β]
    equilibria=beta>0 ? [-sqrt(beta),0,sqrt(beta)] : [0.0]
...
# ╔═╡ 473e7195-e6a2-5595-a98c-14075eabfe34
md"""
...
# ╔═╡ b56a5396-d205-575b-bbb0-dbb69e527875
...
# ╔═╡ deb2d273-a311-59c2-95c9-3da678bf9e0f
md"""
initial prey density: $(@bind course_prey0 Slider(0.2:0.05:2.0; default=0.6, show_value=true))
"""

# ╔═╡ 52f79859-981c-5db7-8505-4128e73b742e
course_lv_u0 = [course_prey0,0.6]

# ╔═╡ 69338fa9-b67a-5ab2-a2db-4a27d90c8b37
course_lv_tspan = (0.0,40.0)

# ╔═╡ 17b73609-3583-5b69-8464-302ea56f6d08
course_lv_p = [1.0,1.0,1.0,1.0]

# ╔═╡ dd408330-c111-5dea-8bc1-61f3e1b0b89f
course_lv_prob = ODEProblem(course_lv!, course_lv_u0, course_lv_tspan, course_lv_p)

# ╔═╡ ab725296-dc7c-5f5e-a869-a38e0caa9822
course_lv_sol = solve(course_lv_prob, Tsit5(); abstol=1e-9, reltol=1e-8);

# ╔═╡ 455c6065-9506-5042-983f-2c77b57af74d
let
    a,b,c,d=course_lv_p
...
# ╔═╡ a57497a9-28ed-5ebb-9bd0-9f8bf008e06b
let
    a,b,c,d=course_lv_p
    x=course_lv_sol[1,:]
    y=course_lv_sol[2,:]
...
# ╔═╡ 297db4bf-7835-5da5-bfc2-7cd38025dbb6
md"""
...
# ╔═╡ 991fc75f-9b98-5d20-bd20-23d0d7d9679b
...
# ╔═╡ 99a10b4b-4dca-534d-be72-d11850f5436a
md"""
β (γ = 1): $(@bind course_infection Slider(0.5:0.1:4.0; default=2.0, show_value=true))
"""

# ╔═╡ e19c9d45-c15f-5c4e-90d8-e144db2bbd6c
course_sir_u0 = [0.99,0.01]

# ╔═╡ 276adcb2-7e08-5713-946f-3cdb95619f65
course_sir_tspan = (0.0,20.0)

# ╔═╡ ee8d77f0-cc87-5db6-9f6b-c8f497f1e53c
course_sir_p = [course_infection,1.0]

# ╔═╡ b8202ddb-59a0-571f-ac3a-907596ee76c3
course_sir_prob = ODEProblem(course_sir!, course_sir_u0, course_sir_tspan, course_sir_p)

# ╔═╡ 35e9878f-f4ca-5936-9ce8-a3d61856c9f4
course_sir_sol = solve(course_sir_prob, Tsit5(); abstol=1e-9, reltol=1e-8);

# ╔═╡ 104d8233-8522-5efc-9188-d7df86904f6a
...
# ╔═╡ fbf97a7c-1c30-53fd-ae2d-0b143c486308
md"""
