...
begin
    ...
    Pkg.instantiate()
    using NonLinearDynamics: flow2d_vectorfield, flow2d_nullclines
...
md"""
For $\gamma>0$ all trajectories approach the origin. They spiral when $\gamma^2<4k$; for stronger damping they approach without oscillating:
...
begin
	prob2 = ODEProblem(reed!, [0.0,0.0], (0,20.0), [μ4,K4])
...
begin
	ticks
	sol0 = solve(ODEProblem(f1.f,f1.x0,(f1.t,f1.t+tail),[k]))
	f1.x0 = sol0.u[end]
	f1.t = sol0.t[end]
	plot_state_flow2D!(p0,f1.x0,f1.t;alpha=0.25)
	#plot!(p0b,sol0.t,getindex.(sol0.u,1),c=:blue)
	#plot!(p0b,[0,sol0.t[end]],[0,0],c=:black)
	plot(p0,size=(600,600))
end	

# ╔═╡ 21309d8e-583c-5f7e-98ac-fad316e1cdad
md"""
## Nullclines
...
# ╔═╡ 2d090a97-fc64-5f7f-af96-19f20b8bb634
let parameters=[k2,γ]
    f=flow2d_nullclines(oscharmdamp!,parameters;xlims=[-1.5,1.5],ylims=[-1.5,1.5],
        regions=false,vectorfield=true,title="Nullclines and a trajectory")
    plot!(f,sol2;idxs=(1,2),color=:black,linewidth=2)
    scatter!(f,[0],[0];color=:black,markersize=4)
end

# ╔═╡ 6b546bf5-2b1a-51a6-9861-b7a72613a41a
md"""
...
**Try:** double the initial displacement, then add damping. Which changes
the amplitude, which changes its long-time destination, and which changes
the period?
"""

# ╔═╡ be2efaca-d3b5-50ce-a620-2f53958d776a
let parameters=[k2,γ], s=sol2
