...
All positive initial conditions converge to this attractor; $x=0$ remains an equilibrium, and
the only effect of $r$ is only on the rate of this convergence.

For autonomous flows on the real line, every bounded trajectory approaches a fixed point; periodic orbits are impossible.
"""
...
function plot_state_flow(x,xrange,t)
	title = "t=$(round(t,digits=4))  x(t) = $(round(x,digits=8))"
...
begin
	reset3
	f3=Flow(cooling,Float64(T0),0.0,[])
...
# ╔═╡ 7aa8a854-7833-5044-a6ee-4594d0fa1c6d
md"""
## Flow, potential, and time trace

For a one-dimensional autonomous flow we can define $V'(x)=-f(x)$.
Then $\dot V=V'(x)\dot x=-f(x)^2\leq0$: motion goes downhill.
For the logistic flow already defined above,

```math
f(x)=r_0x(1-x),\qquad V(x)=r_0\left(\frac{x^3}{3}-\frac{x^2}{2}\right).
```

The minimum at $x=1$ attracts; the maximum at $x=0$ repels. These are scalar
potentials, not the mechanical energy of an oscillator. Follow the same initial
condition in all three views; predict its direction of motion from the slope of $V$.
"""

# ╔═╡ 4ceeaba5-7d04-5c54-9663-5862a189158a
let xs=range(-0.2,2; length=401)
    field=r0.*xs.*(1 .-xs)
    potential=r0.*(xs.^3 ./3 .-xs.^2 ./2)
    solution=solve(ODEProblem(logistic,x02,(0.0,10/r0),[r0]),Tsit5();
        abstol=1e-9,reltol=1e-8)
    a=plot(xs,field; xlabel="x",ylabel="f(x)",legend=false)
    hline!(a,[0];color=:gray)
    b=plot(xs,potential; xlabel="x",ylabel="V(x)",legend=false)
    scatter!(b,[x02],[r0*(x02^3/3-x02^2/2)];color=:black)
    c=plot(solution; xlabel="t",ylabel="x(t)",legend=false)
