...
For $1<r<3$, initial conditions in $0<x_0<1$ (except points that land exactly on an unstable fixed point) converge to an intermediate value. This is also a fixed point and attractor. For example, for $r=2$ the fixed point is at $x_*=0.5$ (check it out!).
...
begin
	nr = argmin(abs.(rs .- R2))
	if minimum(rs) <= R2 <= maximum(rs)
		plot!(deepcopy(p3),rs[nr:nr],x[nr:nr,1:maxiter],seriestype = :scatter, markercolor=:red, markersize = 3, markerstrokewidth = 0, markeralpha = 0.3)
	else
		p3
	end
end	
...
# ╔═╡ e47927cd-63e8-54aa-b1a1-6b2be694c001
md"""
## Read an iteration geometrically

For the logistic map $x_{n+1}=r x_n(1-x_n)$, go vertically to the graph of
$f(x)$ and horizontally to the diagonal. Each pair of steps is one iteration.
The same construction will reappear as a **return map** of a continuous flow.

Predict the destination before moving the existing $r$ slider. At a fixed point,
$|f'(x_*)|<1$ attracts nearby iterates; a negative slope makes them alternate
around it. This differs from a flow, where the sign of $f'(x_*)$ determines stability.
"""

# ╔═╡ ebdf3bb7-53ae-56f0-97d2-365f8db3829e
let r=R2, state=x02
    xs=range(0,1; length=401)
    fig=plot(xs,r.*xs.*(1 .-xs); label="f(x)", xlabel="xₙ", ylabel="xₙ₊₁",
        size=(550,450),margin=5*Plots.mm, xlims=(0,1), ylims=(0,1))
    plot!(fig,xs,xs; color=:gray, label="diagonal")
    cx=Float64[state]; cy=Float64[0]
    for _ in 1:40
        next=r*state*(1-state)
        append!(cx,[state,next]); append!(cy,[next,next])
        state=next
    end
    plot!(fig,cx,cy; label="40 iterations", linewidth=1, color=:darkorange)
end
