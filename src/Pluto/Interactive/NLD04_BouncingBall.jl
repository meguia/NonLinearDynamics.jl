...
md"""
h(0) $(@bind h0 Slider(0.5:0.5:50,default=30.0;show_value=true))
...
begin
	u1 = [0.5,0.85,0.0,0.0]
	u2 = [0.5-10.0^δ,0.85,0.0,0.0]
	tspan = (0.0,tmax)
	g = 1.0
	sol1 = solve(ODEProblem(ballcirclehard!,u1,tspan,[g]); callback=ContinuousCallback(collision,nothing,bounce!), abstol=1e-12, reltol=1e-10)
	sol2 = solve(ODEProblem(ballcirclehard!,u2,tspan,[g]); callback=ContinuousCallback(collision,nothing,bounce!), abstol=1e-12, reltol=1e-10)
end;
...
begin
	...
	dd = Array((sol1(ts,idxs=2)-sol2(ts,idxs=2)).^2+(sol1(ts,idxs=1)-sol2(ts,idxs=1)).^2)
	plot(ts,0.5*log10.(max.(dd,floatmin(Float64))),size=(800,300),legend=false,xlabel="time",ylabel="log₁₀  distance")
end	

# ╔═╡ 9b3968f7-ea49-56c8-ad22-131a7fe88ba0
md"""
...
# ╔═╡ 4ab2b80a-0a47-5449-8d66-a0057fd32d75
...
# ╔═╡ 107ae35a-ca2e-5c93-80d9-b70c19f83023
md"""
restitution e: $(@bind course_restitution Slider(0.85:0.01:1.0; default=0.9, show_value=true))
"""

# ╔═╡ 5e7bf406-778b-5f1e-bb7e-de4a2eaebaa2
...
# ╔═╡ a1ae048f-498f-5140-9f16-5aee87f84c8a
md"""
