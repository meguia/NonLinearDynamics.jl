...
md"""
Blowing γ $(@bind γ Slider(0.0:0.01:2.0; default=0.4, show_value=true))
Opening ζ $(@bind ζ Slider(0.0:0.01:1.0; default=0.2, show_value=true))\
First resonance Ω₁ $(@bind Ω1 Slider(0.5:0.01:2.0; default=1.0, show_value=true))
Q1 $(@bind Q1 Slider(1.0:0.1:50.0; default=30.0, show_value=true)) \
Second resonance Ω₂ $(@bind Ω2 Slider(1.0:0.01:3.0; default=2.5, show_value=true))
Q2 $(@bind Q2 Slider(1.0:0.1:50.0; default=30.0, show_value=true)) \
...
p = [γ, ζ, 2pi*2000/1161.2, Ω1, Ω2, Q1, Q2, 1322/1161.2, 2386/1161.2, 1e-4]
...
let
    ...
    plot(spectrum.freq, 10 .* log10.(max.(relative_power, 1e-10));
        xlims=(0, 1000*3f0), ylims=(-80, 5), xlabel="frequency (Hz)", ylabel="relative power (dB)", legend=false)
end
...
# ╔═╡ 85bcf8ef-6a4c-53aa-af45-edac65090bc8
md"""
## From a torus to an audible beating pattern

**Predict → listen → explain:** compare the periodic and quasiperiodic
presets after the discarded transient. Relate the Poincaré points to the
circle and torus lesson: a finite set suggests periodic locking, while a
resolved closed curve is consistent with a torus. Check the spectrum for
two independent frequencies and their combinations; two harmonics of one
fundamental alone do not establish quasiperiodicity.
"""
