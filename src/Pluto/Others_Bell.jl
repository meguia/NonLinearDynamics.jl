...
$F_c = \frac{1}{2} k (\phi-\phi_{max})^2  \quad : \quad|\phi|>\phi_{max}$
...
function bounce!(integrator)
	...
	if integrator.dt > 1e-12
        ...
        integrator.u[4] = 0
        integrator.u[3] = sign(u[3])*26*pi/180
    end
end	
...
solh = solve(probh, callback=ContinuousCallback(collision,bounce!),Rosenbrock23());
...
tmax $(@bind tmax Slider(5.0:1.0:100.0,default=0.0;show_value=true))
"""
